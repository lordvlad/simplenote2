...
(function($) {
  jQuery.fn.contextMenu = function ( name, actions, options ) {
    ...
    default_options = {
      ...
      pass_native_context_menu_on_alt_shift_ctrl : true // allow native context menu on with alt/shift/ctrl
    },
    ...
    $.each(actions, function(me, itemOptions) {
      var link;
      if ( $.isEmptyObject( itemOptions ) ){
        link = me;
      }
      else if (itemOptions.link) {
        link = itemOptions.link;
      } else {
        link = '<a onmousedown="event.preventDefault();" href="javascript:void(0);">'+me+'</a>';
      }
    ...
    var mouseEventFunc = function(e){
      // pass event on ctrl/alt/shift
      if (options.pass_native_context_menu_on_alt_shift_ctrl && (e.altKey||e.shiftKey||e.ctrlKey) ){ return; }
