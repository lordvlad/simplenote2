...
(function() {
  var Node, SimpleNote, Tag, author, date, delay, email, github, hash, intersect, interval, isArr, isDate, isFn, isNum, isObj, isStr, koMap, maxScreenWidthForMobile, now, obs, revision, revive, sizeOf, store, time, timeout, uuid, version, _ref,
  ...
  koMap = function(model, map) {
    ...
    for (key in map) {
      if (isObj(model[key])) {
        koMap(model[key], map[key]);
      } else if (ko.isWriteableObservable(model[key])) {
        model[key](map[key]);
      } else {
        model[key] = map[key];
      }
  ...
  */
  ...
  SimpleNote.prototype.attachEvents = function() {
    ...
    $('#search > div > .icon-ellipsis-vertical').on('click.special', function(e) {
      var s;
    ...
    return (function() {
      var eventToBits, h, hotkeybits, keys, keysToBits, stop, _i, _len, _ref2;
      ...
      keysToBits = function(a) {
        ...
        return [a.match(/alt/i) ? 1 : 0, a.match(/ctrl/i) ? 1 : 0, a.match(/shift/i) ? 1 : 0, ((c = a.match(/(?:ctrl|alt|shift|\s)*(?:\s|^)(\w{2,})(?=\s|$)/i)) && keys[c[1].toUpperCase()]) || ((c = a.match(/(?:ctrl|alt|shift|\s)*(?:\s|^)(\w{1})(?=\s|$)/i)) && (c = c[1].toUpperCase().charCodeAt(0)))].join("");
      };
      hotkeybits = {};
      _ref2 = model.options.hotkeys;
      ...
      return $doc.on('keydown.hotkeys', function(e) {
        var k, _base, _j, _len1, _name, _results;

        if ((h = hotkeybits[eventToBits(e)])) {
          _results = [];
          for (_j = 0, _len1 = h.length; _j < _len1; _j++) {
            k = h[_j];
            if (!k[0] || $(e.target).is(k[0])) {
              stop(e);
              _results.push(typeof (_base = model.functions)[_name = k[1]] === "function" ? _base[_name](e) : void 0);
            } else {
  ...
  SimpleNote.prototype.functions = {
    next: function(e) {
      ...
      if ((t = $(e.target).findNext('.title:visible')[0])) {
        return ko.dataFor(t).active(true);
      }
    },
    prev: function(e) {
      var t;
  ...
  SimpleNote.prototype.options = {
    ...
    hotkeys: [['ctrl j', null, 'next'], ['down', null, 'next'], ['ctrl k', null, 'prev'], ['up', null, 'prev'], ['esc', null, 'focusSearch'], ['esc', '#search input', 'blurFocus']]
  };
