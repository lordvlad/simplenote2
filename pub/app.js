...
(function() {
  var $, $doc, $vm, $win, Bookmark, MAXSCREENWIDTHFORMOBILE, Node, SimpleNote, Tag, author, date, delay, email, github, hash, instance, intersect, interval, isArr, isDate, isFn, isNum, isObj, isStr, jump, keys, now, obs, revision, revive, sizeOf, store, time, timeout, title, uuid, version, vm, _ref,
  ...
  SimpleNote = (function() {
    ...
    SimpleNote.syncModulesCount = null;
    ...
    function SimpleNote(element) {
      ...
      this.$.on('initialized', function() {
        return $win.trigger('appInitialized', _this);
      });
      this.$.on('appsetupready', function() {});
      this.$.on('ready', function() {
        return $win.trigger('appReady', _this);
      });
      this.$.on('online, offline', function() {
        return console.log('app on/offline', arguments);
      });
      this.timeout = null;
      this.revived = false;
      this.interval = null;
      ...
      this.breadcrumbs = obs(function() {
        ...
        return ((_ref1 = _this.currentNode()) != null ? typeof _ref1.parents === "function" ? _ref1.parents().concat(a) : void 0 : void 0) || [];
      });
      hash.subscribe(function(id) {
        var _ref1;

        _this.currentNode((id && id.length && _this.nodes.find("id", id)) || (id && id.length && hash('') && _this.root) || _this.root);
        if ((_ref1 = _this.currentNode()) != null) {
          _ref1.editingNote(true);
        }
  ...
  instance = function() {
    return SimpleNote.activeInstance;
  };

  (function() {
    var connect, disconnect, load, localStorageModule, module, save, status, statusDef, subscription,
      _this = this;

    statusDef = [['not&nbsp;set&nbsp;up', ''], ['ready', 'green'], ['syncing', 'yellow'], ['disconnected', 'red']];
    localStorageModule = {
      name: 'localStorage'
    };
    ...
    disconnect = null;
    ({
      activate: function(mod) {},
      status: status,
      module: module,
      connect: function() {
        if (!connect && (module = store.get('syncModule')) && (module = SimpleNote.syncModules[module])) {
          activate(module);
        }
        if (connect && instance()) {
          return connect.apply(instance(), arguments);
        }
      },
      disconnect: function() {
        store.remove('syncModule');
        if (disconnect && instance()) {
          return disconnect.apply(instance(), arguments);
        }
      }
    });
    status = obs(0);
    ...
    SimpleNote.prototype.connect = function(module) {
      var mod, _base;

      if (module == null) {
        module = store.get('syncModule');
      }
      if (typeof (_base = this.sync).disconnect === "function") {
        _base.disconnect();
      }
      console.log(module);
      if (typeof mod === 'string') {
        mod = SimpleNote.syncModules[mod];
      }
      store.set('syncModule', module.name);
      subscription = module.status.subscribe(this.status);
      module.status.valueHasMutated();
      save = module.save;
      load = module.load;
      connect = module.connect;
      return disconnect = module.disconnect;
    };
    SimpleNote.prototype.load = function(callback) {
      if (load && instance()) {
        return load.call(instance());
      } else {
    ...
    return SimpleNote.prototype.save = function(callback) {
      if (!this.revived) {
        return;
      }
      ...
      if (save && instance()) {
        return save.call(instance(), callback);
      } else {
  ...
  SimpleNote.syncModules.dropbox = (function() {
    var cfg, client, connect, disconnect, load, save, saveDelayed, signIn, status, timer,
      _this = this;

    status = obs(0);
    client = timer = null;
    saveDelayed = function(obj, callback) {
      return timeout.reset(timer, 1000, function() {
    ...
    save = function(callback) {
      ...
      if (!obj || !callback || !client) {
        return;
      }
      client.writeFile('root', JSON.stringify(obj.root, null, 2), function(e, s) {
        return add('root', e, s);
      });
      client.writeFile('bookmarks', JSON.stringify(obj.bookmarks, null, 2), function(e, s) {
        return add('bookmarks', e, s);
      });
      client.writeFile('tags', JSON.stringify(obj.tags, null, 2), function(e, s) {
        return add('tags', e, s);
      });
      client.writeFile('archive', JSON.stringify(obj.archive, null, 2), function(e, s) {
        return add('archive', e, s);
      });
      return client.writeFile('options', JSON.stringify(obj.options, null, 2), function(e, s) {
    ...
    load = function(callback) {
      ...
      if (!client) {
        callback(new Error('dropbox client not initialized'), null);
      }
      ans = {};
      add = function(k, e, s) {
        if (e) {
      ...
      client.readFile('root', function(e, s) {
        return add('root', e, s);
      });
      client.readFile('bookmarks', function(e, s) {
        return add('bookmarks', e, s);
      });
      client.readFile('tags', function(e, s) {
        return add('tags', e, s);
      });
      client.readFile('archive', function(e, s) {
        return add('archive', e, s);
      });
      return client.readFile('options', function(e, s) {
        return add('options', e, s);
      });
    };
    cfg = {
    ...
    signIn = function(callback, skip) {
      if (skip || confirm('you will be redirected to the dropbox sign in page. after signing and allowing simplenote to access your dropbox, you will be redirected back to simplenote. you will see the updated status on the bottom of the page.')) {
    ...
    connect = function(callback) {
      ...
      if (typeof (_base = instance()) === "function") {
        _base(connect('dropbox'));
      }
      ...
      return client.authenticate(function(e) {
        status(e ? 1 : 2);
        return callback([].slice.call(arguments, 1));
      });
    };
    disconnect = function(callback) {
      if (client && client.signOff) {
        return client.signOff(function() {
          ...
          return callback(null, 'dropbox disconnected successfully');
        });
      } else {
        return callback(null);
      }
  ...
  */
  ...
  Node = (function() {
    function Node(options) {
      ...
      this.visible = obs(function() {
        ...
        _ref1 = vm.realFilter(), tags = _ref1.tags, words = _ref1.words;
  ...
  $(window).on({
    'noupdate': function() {
      return console.log('appcache is bubbling');
    },
    'online, offline': function() {
      var _ref1;

      return (_ref1 = $vm.trigger) != null ? _ref1.apply(vm, arguments) : void 0;
    },
    ...
    'appReady': function() {
      console.log(this);
      return $('#curtain').show(1).delay(1).fadeOut('slow');
    }
  });
